...
import { Plus, Edit, X, Loader2, Search, PowerOff, Building2, ChevronLeft, ChevronRight } from 'lucide-react';
import toast from 'react-hot-toast';
import api from '../../services/api.js';

const svcApi = {
  getAll:  ()         => api.get('/servicios-terceros').then(r => r.data.data),
  create:  (data)     => api.post('/servicios-terceros', data).then(r => r.data.data),
  update:  (id, data) => api.put(`/servicios-terceros/${id}`, data).then(r => r.data.data),
};
...
const btnP         = { flex: 1, padding: '10px 0', borderRadius: 10, border: 'none', cursor: 'pointer', background: '#7C3AED', color: '#fff', fontSize: 13, fontWeight: 600, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6 };
...
// ─── Modal ────────────────────────────────────────────────────────
function ModalServicioTercero({ servicio, onClose, onSaved }) {
  const editando = !!servicio;
  const [form, setForm] = useState({ nombre: servicio?.nombre || '', descripcion: servicio?.descripcion || '', precio: servicio?.precio || '' });
  ...
  return (
    <div style={overlayStyle} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={cardStyle}>
        <div style={mHeader}>
          <div>
            <h2 style={{ fontSize: 15, fontWeight: 700, color: '#0D1B2A', margin: 0 }}>{editando ? 'Editar servicio externo' : 'Nuevo servicio externo'}</h2>
            <p style={{ fontSize: 12, color: '#94A3B8', marginTop: 3 }}>Servicios realizados por terceros</p>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={mBody}>
            ...
            <div style={fg}>
              ...
              <input style={iStyle} value={form.nombre} onChange={e => set('nombre', e.target.value)} required placeholder="ej: Enllantado" onFocus={focus} onBlur={blur} />
            </div>
            <div style={fg}>
              <label style={lStyle}>Descripción</label>
              <textarea style={{ ...iStyle, resize: 'vertical', minHeight: 80 }} value={form.descripcion} onChange={e => set('descripcion', e.target.value)} placeholder="Descripción del servicio externo..." onFocus={focus} onBlur={blur} />
            </div>
            <div style={{ ...fg, marginBottom: 0 }}>
              <label style={lStyle}>Precio (S/) *</label>
              <input style={iStyle} type="number" step="0.01" min="0" value={form.precio} onChange={e => set('precio', e.target.value)} required placeholder="0.00" onFocus={focus} onBlur={blur} />
            </div>
          </div>
          <div style={mFooter}>
...
// ─── Página ───────────────────────────────────────────────────────
export default function ServiciosTerceros() {
  const qc = useQueryClient();
  const [modal,  setModal]  = useState(null);
  const [search, setSearch] = useState('');
  const [page,   setPage]   = useState(1);

  const { data: servicios = [], isLoading } = useQuery({ queryKey: ['servicios-terceros'], queryFn: svcApi.getAll });
  ...
  const onSaved = () => { qc.invalidateQueries(['servicios-terceros']); setModal(null); };

  const handleToggle = async (s) => {
    if (!confirm(`¿${s.activo ? 'Desactivar' : 'Activar'} "${s.nombre}"?`)) return;
    try {
      await svcApi.update(s.id, { activo: !s.activo });
      toast.success(s.activo ? 'Servicio desactivado' : 'Servicio activado');
      qc.invalidateQueries(['servicios-terceros']);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0D1B2A', margin: 0 }}>Servicios de terceros</h2>
          <p style={{ fontSize: 12, color: '#94A3B8', marginTop: 2 }}>{filtrados.length} de {servicios.length} servicios externos</p>
        </div>
        <button onClick={() => setModal({})}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 10, border: 'none', cursor: 'pointer', background: '#7C3AED', color: '#fff', fontSize: 13, fontWeight: 600 }}
          onMouseEnter={e => e.currentTarget.style.background = '#6D28D9'}
          onMouseLeave={e => e.currentTarget.style.background = '#7C3AED'}>
          <Plus size={15} /> Nuevo servicio externo
        </button>
      </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#F8FAFC', borderBottom: '1px solid #F1F5F9' }}>
                    {['Servicio', 'Descripción', 'Precio', 'Estado', 'Acciones'].map(h => (
                ...
                <tbody>
                    ...
                    <tr key={s.id} style={{ borderBottom: '1px solid #F8FAFC', opacity: s.activo ? 1 : 0.45 }}
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 32, height: 32, borderRadius: 10, background: '#F5F3FF', border: '1px solid #DDD6FE', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{ fontSize: 14, fontWeight: 700, color: '#0D1B2A' }}>{formatMoney(s.precio)}</span>
                      </td>
                  ...
                  {!paginated.length && (
                    <tr><td colSpan={5} style={{ textAlign: 'center', padding: 48, color: '#94A3B8', fontSize: 13 }}>
                      {search ? 'Sin resultados para la búsqueda' : 'No hay servicios externos registrados'}
                    </td></tr>
                  )}
                </tbody>
              </table>
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((p, i) => p === '...'
                      ...
                      : <button key={p} onClick={() => setPage(p)}
                          style={{ width: 32, height: 32, borderRadius: 8, border: `1px solid ${p === page ? '#7C3AED' : '#E2ECF4'}`, background: p === page ? '#7C3AED' : '#fff', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: p === page ? 700 : 400, color: p === page ? '#fff' : '#64748B' }}>{p}</button>
                    )}
      ...
      {modal !== null && (
        <ModalServicioTercero servicio={modal.id ? modal : null} onClose={() => setModal(null)} onSaved={onSaved} />
      )}
