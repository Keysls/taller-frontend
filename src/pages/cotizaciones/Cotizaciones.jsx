...
import {
  ...
  FileText, Printer, ClipboardList, Eye,
} from 'lucide-react';
...
function ModalEjecutarOT({ cotizacion, onClose, onDone }) {
  ...
  const [form, setForm] = useState({
    // Datos cliente/vehículo (solo lectura desde cot)
    ...
    <div style={{...ov,zIndex:300}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:'#fff',borderRadius:20,width:'100%',maxWidth:700,boxShadow:'0 24px 80px rgba(15,23,42,0.2)',border:'1px solid #E2ECF4',display:'flex',flexDirection:'column',maxHeight:'92vh',overflow:'hidden'}}>
        ...
        <div style={{flex:1,overflowY:'auto'}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  {[['Señor(a)',form.clienteNombre,true],['Teléfono',form.clienteTel],['Método de pago',form.metodoPago]].map(([l,v,b])=>(
                    <div key={l}>
                      ...
                      <input style={{...iS,background:'#F1F5F9',color:'#64748B'}} value={v} readOnly/>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px 12px'}}>
                  {[['Placa','placa'],['Marca','marca'],['Modelo','modelo'],['Motor','motor'],['Kilometraje','km']].map(([l,k])=>(
                    <div key={k}>
                      <label style={{...lS}}>{l}</label>
                      <input style={iS} value={form[k]||''} onChange={e=>set(k,e.target.value)}
            ...
            <div style={{padding:'16px 20px'}}>
              ...
              {serviciosItems.length>0&&(
                <div style={{marginBottom:16}}>
                  <div style={{fontSize:12,fontWeight:700,color:'#1D4ED8',marginBottom:8}}>Servicios propios</div>
                  {serviciosItems.map((item,i)=>(
                    <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #F8FAFC',fontSize:13}}>
                      <span>{item.descripcion}</span><span style={{fontWeight:600}}>{fmt(item.precioUnit)}</span>
                    </div>
                  ))}
                </div>
              )}
              {tercerosItems.length>0&&(
                <div style={{marginBottom:16}}>
                  <div style={{fontSize:12,fontWeight:700,color:'#7C3AED',marginBottom:8}}>Servicios de terceros</div>
                  {tercerosItems.map((item,i)=>(
                    <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #F8FAFC',fontSize:13}}>
                      <span>{item.descripcion}</span><span style={{fontWeight:600}}>{fmt(item.precioUnit)}</span>
                    </div>
                  ))}
              ...
              {repuestosItems.length>0&&(
                <div style={{marginBottom:16}}>
                  ...
                  {repuestosItems.map((item,i)=>(
                    <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #F8FAFC',fontSize:13}}>
                      <span>{item.descripcion} {item.cantidad>1?`(${item.cantidad}x)`:''}</span>
          ...
          {/* Tab 3 — Orden de Trabajo */}
          {step===3 && (
            <div style={{padding:'20px 24px'}}>
              <div style={{display:'flex',flexDirection:'column',gap:14}}>
                <div>
                  <label style={{...lS}}>Mecánico asignado</label>
                  <select style={iS} value={form.mecanicoId} onChange={e=>set('mecanicoId',e.target.value)} onFocus={focus} onBlur={blur}>
                    <option value="">Sin asignar</option>
                    {mecanicos.map(m=><option key={m.id} value={m.id}>{m.nombre} — {m.estado}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{...lS}}>Diagnóstico</label>
                  <textarea style={{...iS,resize:'vertical',minHeight:80}} value={form.diagnostico} onChange={e=>set('diagnostico',e.target.value)} placeholder="Diagnóstico inicial..." onFocus={focus} onBlur={blur}/>
                </div>
                <div>
                  <label style={{...lS}}>Observaciones</label>
                  <textarea style={{...iS,resize:'vertical',minHeight:72}} value={form.observaciones} onChange={e=>set('observaciones',e.target.value)} placeholder="Observaciones adicionales..." onFocus={focus} onBlur={blur}/>
                </div>
                <div>
        ...
        <div style={{padding:'14px 20px',borderTop:'1px solid #E2ECF4',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
          <button onClick={()=>step>1?setStep(step-1):onClose()}
            style={{width:36,height:36,borderRadius:10,border:'1px solid #E2ECF4',background:'#fff',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'#64748B',fontSize:18}}>
            ←
          </button>
          <button onClick={()=>step<3?setStep(step+1):handleCrear()} disabled={loading}
            style={{padding:'9px 24px',borderRadius:10,border:'none',background:step===3?'#059669':'#2563EB',color:'#fff',cursor:'pointer',fontSize:13,fontWeight:700,display:'inline-flex',alignItems:'center',gap:6}}>
...
function ModalVerCotizacion({ id, onClose, onEjecutar }) {
  ...
  if (isLoading) return (
    <div style={ov}>
      <div style={{background:'#fff',borderRadius:20,padding:48,display:'flex',alignItems:'center',gap:12,color:'#94A3B8'}}>
  ...
  // Calcular subtotales y descuento implícito
  const subtotalSvc = [...serviciosItems, ...tercerosItems].reduce((s,i) => s + Number(i.precioUnit||0), 0);
  const subtotalRep = repuestoItems => repuestoItems.reduce((s,i) => s + Number(i.subtotal||0), 0);
  ...
  return (
              ...
              <div style={{borderTop:'1px solid #F1F5F9',padding:'0 18px 16px'}}>
                ...
                {serviciosItems.length > 0 && (
                  <div style={{marginTop:14}}>
                    <div style={{fontSize:12,fontWeight:700,color:'#1D4ED8',marginBottom:8}}>Servicio propio</div>
                    {serviciosItems.map((item,i)=>(
                      <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'5px 0',fontSize:13,borderBottom:'1px solid #F8FAFC'}}>
                        <span style={{color:'#1E293B'}}>{item.descripcion}</span>
                        <span style={{fontWeight:600}}>{fmt(item.precioUnit)}</span>
                      </div>
                    ))}
                  </div>
                )}
                    ...
                    {repuestosItems.map((item,i)=>(
                      <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'5px 0',fontSize:13,borderBottom:'1px solid #F8FAFC'}}>
                        ...
                        <div style={{display:'flex',alignItems:'center',gap:8}}>
                          {item.cantidad > 1 && <span style={{fontSize:11,color:'#94A3B8'}}>{item.cantidad}x</span>}
                          <span style={{fontWeight:600}}>{fmt(item.subtotal||item.precioUnit)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {descuento > 0 && (
                  <div style={{display:'flex',justifyContent:'space-between',padding:'8px 0',marginTop:8,borderTop:'1px solid #F1F5F9',fontSize:13,color:'#DC2626'}}>
                    <span>Descuento</span>
                    <span style={{fontWeight:600}}>- {fmt(descuento)}</span>
                  </div>
                )}
              </div>
            )}
    ...
    {showOT && (
      <ModalEjecutarOT
        cotizacion={cot}
        onClose={()=>setShowOT(false)}
        onDone={()=>{ setShowOT(false); onEjecutar(); }}
      />
    )}
    {showModificar && (
      <NuevaCotizacionModal
        ...
        onSaved={()=>{ setShowModificar(false); onEjecutar(); }}
      />
    )}
...
function ModalNuevaCotizacion({ onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await cotApi.create({ ...form, items, total });
    ...
    <div style={ov} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:'#fff',borderRadius:20,width:'100%',maxWidth:600,boxShadow:'0 20px 60px rgba(30,58,138,0.15)',border:'1px solid #E2ECF4',display:'flex',flexDirection:'column',maxHeight:'92vh',overflow:'hidden'}}>
        ...
        <div style={{padding:'20px 24px',borderBottom:'1px solid #F1F5F9',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
          <div>
            ...
            <div style={{display:'flex',gap:6,marginTop:6}}>
              {['Información','Ítems'].map((s,i)=>(
                <span key={s} style={{fontSize:11,fontWeight:600,padding:'2px 10px',borderRadius:20,
                  background: step===i+1 ? '#2563EB' : '#F1F5F9',
        ...
        <div style={{overflowY:'auto',flex:1,padding:'20px 24px'}}>
            ...
            <>
              <div style={r2}>
                <div>
                  ...
                  <select style={iS} value={form.clienteId} onChange={e=>{ set('clienteId',e.target.value); set('vehiculoId',''); }} onFocus={focus} onBlur={blur}>
                    <option value="">Seleccionar...</option>
                    {clientes.map(c=><option key={c.id} value={c.id}>{c.nombres} {c.apellidos} — {c.dniRuc}</option>)}
                  </select>
                </div>
                <div>
                  <label style={lS}>Vehículo *</label>
                  <select style={iS} value={form.vehiculoId} onChange={e=>set('vehiculoId',e.target.value)} onFocus={focus} onBlur={blur} disabled={!form.clienteId}>
                    <option value="">Seleccionar...</option>
                    {vehCliente.map(v=><option key={v.id} value={v.id}>{v.placa} — {v.marca} {v.modelo}</option>)}
                  </select>
                </div>
              </div>
              <div style={r2}>
                <div>
                  ...
                  <select style={iS} value={form.metodoPago} onChange={e=>set('metodoPago',e.target.value)} onFocus={focus} onBlur={blur}>
                    {METODOS_PAGO.map(m=><option key={m} value={m}>{m}</option>)}
                  </select>
                </div>
                <div>
                  <label style={lS}>Motor</label>
                  <input style={iS} value={form.motor} onChange={e=>set('motor',e.target.value)} placeholder="ej: 2.0L" onFocus={focus} onBlur={blur}/>
                </div>
              </div>
              <div style={fg}>
                <label style={lS}>Observaciones</label>
                <textarea style={{...iS,resize:'vertical',minHeight:72}} value={form.observaciones} onChange={e=>set('observaciones',e.target.value)} onFocus={focus} onBlur={blur}/>
              </div>
            </>
          )}
          ...
          {step===2 && (
            <>
              {/* Repuestos */}
              <div style={{marginBottom:16}}>
                <div style={{fontSize:11,fontWeight:700,color:'#B45309',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:8}}>Repuestos</div>
                <select style={{...iS,marginBottom:8}} defaultValue="" onChange={e=>{
                  const r = repuestos.find(x=>x.id===e.target.value);
                  if(r){ addItem('repuesto',r,1); e.target.value=''; }
                }} onFocus={focus} onBlur={blur}>
                  <option value="">+ Agregar repuesto...</option>
                  {(Array.isArray(repuestos)?repuestos:repuestos?.data||[]).filter(r=>r.activo&&r.stock>0).map(r=><option key={r.id} value={r.id}>{r.nombre} — Stock: {r.stock}</option>)}
                </select>
              </div>

              {/* Servicios propios */}
              <div style={{marginBottom:16}}>
                <div style={{fontSize:11,fontWeight:700,color:'#1D4ED8',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:8}}>Servicios propios</div>
                <select style={{...iS,marginBottom:8}} defaultValue="" onChange={e=>{
                  const s = servicios.find(x=>x.id===e.target.value);
                  if(s){ addItem('servicio',s); e.target.value=''; }
                }} onFocus={focus} onBlur={blur}>
                  <option value="">+ Agregar servicio...</option>
                  {servicios.filter(s=>s.activo).map(s=><option key={s.id} value={s.id}>{s.nombre} — {fmt(s.precioBase)}</option>)}
                </select>
              </div>

              {/* Servicios terceros */}
              <div style={{marginBottom:16}}>
                <div style={{fontSize:11,fontWeight:700,color:'#6D28D9',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:8}}>Servicios de terceros</div>
                <select style={{...iS,marginBottom:8}} defaultValue="" onChange={e=>{
                  const s = terceros.find(x=>x.id===e.target.value);
                  if(s){ addItem('tercero',s); e.target.value=''; }
                }} onFocus={focus} onBlur={blur}>
                  <option value="">+ Agregar servicio externo...</option>
                  {terceros.filter(s=>s.activo).map(s=><option key={s.id} value={s.id}>{s.nombre} — {fmt(s.precio)}</option>)}
                </select>
              </div>
                ...
                <div style={{borderRadius:12,border:'1px solid #F1F5F9',overflow:'hidden',marginBottom:12}}>
                  <table style={{width:'100%',borderCollapse:'collapse'}}>
                    <thead><tr style={{background:'#F8FAFC'}}>
                      {['Tipo','Descripción','Cant.','Precio',''].map(h=>(
                        ...
                        <tr key={i} style={{borderBottom:'1px solid #F8FAFC'}}>
                          <td style={{padding:'8px 12px'}}>
                            <span style={{fontSize:10,fontWeight:700,padding:'2px 8px',borderRadius:20,
                              background: item.tipo==='repuesto'?'#FFF7ED':item.tipo==='servicio'?'#EFF6FF':'#F5F3FF',
                              color:      item.tipo==='repuesto'?'#C2410C':item.tipo==='servicio'?'#1D4ED8':'#6D28D9',
                            }}>{item.tipo}</span>
                          </td>
                          <td style={{padding:'8px 12px',fontSize:13,color:'#0D1B2A'}}>{item.descripcion}</td>
                          <td style={{padding:'8px 12px',fontSize:12,color:'#64748B'}}>{item.cantidad}</td>
                          <td style={{padding:'8px 12px',fontSize:13,fontWeight:600}}>{fmt(item.subtotal||item.precioUnit)}</td>
                          <td style={{padding:'8px 12px'}}>
                            <button onClick={()=>removeItem(i)} style={{background:'none',border:'none',cursor:'pointer',color:'#DC2626',padding:2}}><X size={14}/></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  <div style={{padding:'10px 14px',background:'#F8FAFC',display:'flex',justifyContent:'flex-end',gap:8,alignItems:'center'}}>
                    <span style={{fontSize:12,color:'#64748B'}}>Total:</span>
                    <span style={{fontSize:16,fontWeight:800,color:'#0D1B2A'}}>{fmt(total)}</span>
                  </div>
                </div>
              )}
              {!items.length && (
                <div style={{textAlign:'center',padding:'24px 0',color:'#94A3B8',fontSize:13}}>Agrega repuestos o servicios arriba</div>
              )}
            </>
          )}
        </div>
        ...
        <div style={{padding:'16px 24px',borderTop:'1px solid #F1F5F9',display:'flex',gap:8,justifyContent:'space-between',flexShrink:0}}>
          <button style={BC} onClick={()=>step===1?onClose():setStep(1)}>
            {step===1?'Cancelar':'← Atrás'}
          </button>
          {step===1
            ? <button style={BP} onClick={()=>{ if(!form.clienteId||!form.vehiculoId){toast.error('Selecciona cliente y vehículo');return;} setStep(2); }}>
                Siguiente →
              </button>
            : <button style={BP} onClick={handleSubmit} disabled={loading}>
...
export default function Cotizaciones() {
  ...
  const [modal,   setModal]   = useState(null); // null | { tipo:'ver'|'nueva', id? }
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
        <div>
          <p style={{fontSize:12,color:'#94A3B8',marginTop:2}}>{filtradas.length} de {cotizaciones.length} cotizaciones</p>
        </div>
        <button onClick={()=>setModal({tipo:'nueva'})}
        ...
        <div style={{padding:'14px 16px',borderBottom:'1px solid #F1F5F9',display:'flex',gap:10}}>
          <div style={{position:'relative',flex:1}}>
            ...
            <input value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}}
              ...
              style={{...iS,paddingLeft:32,background:'#F8FAFC'}} onFocus={focus} onBlur={blur}/>
          </div>
              ...
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead>
                  <tr style={{background:'#F8FAFC',borderBottom:'1px solid #F1F5F9'}}>
                    {['Código','DNI/RUC','Cliente','Técnico','Placa','Estado','Total','Fecha',''].map(h=>(
                ...
                <tbody>
                  {paginated.map(c=>{
                    ...
                    return (
                      <tr key={c.id} style={{borderBottom:'1px solid #F8FAFC',cursor:'pointer'}}
                        ...
                        onClick={()=>setModal({tipo:'ver',id:c.id})}>
                        <td style={{padding:'11px 14px'}}>
                          <span style={{fontFamily:'monospace',fontSize:12,fontWeight:700,color:'#2563EB',background:'#EFF6FF',padding:'2px 8px',borderRadius:6}}>{c.numeroCot}</span>
                        </td>
                        ...
                        <td style={{padding:'11px 14px'}}>
                          {(c.placa || c.vehiculo?.placa)
                            ? <span style={{fontSize:12,fontWeight:700,background:'#F1F5F9',color:'#334155',padding:'2px 8px',borderRadius:6}}>{c.placa || c.vehiculo?.placa}</span>
                            : <span style={{color:'#CBD5E1',fontSize:12}}>—</span>}
                        </td>
                        <td style={{padding:'11px 14px'}}>
                          <span style={{fontSize:11,fontWeight:700,padding:'3px 10px',borderRadius:20,...estCfg}}>{c.estado}</span>
                        </td>
                        ...
                        <td style={{padding:'11px 14px'}}>
                          <button onClick={e=>{e.stopPropagation();setModal({tipo:'ver',id:c.id});}}
                  ...
                  {!paginated.length&&(
                    <tr><td colSpan={9} style={{textAlign:'center',padding:48,color:'#94A3B8',fontSize:13}}>
      ...
      {/* Modales */}
      {modal?.tipo==='nueva' && (
        <NuevaCotizacionModal onClose={()=>setModal(null)} onSaved={onSaved}/>
      )}
      {modal?.tipo==='ver'   && (
        <ModalVerCotizacion
          id={modal.id}
          onClose={()=>setModal(null)}
          onEjecutar={()=>{ qc.invalidateQueries(['cotizaciones']); setModal(null); }}
        />
      )}
      <style>{`@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}} @keyframes slideIn{from{transform:translateX(100%)}to{transform:translateX(0)}}`}</style>
    </div>
